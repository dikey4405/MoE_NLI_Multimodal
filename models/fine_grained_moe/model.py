...
class FineGrainedMoENLIModel(nn.Module):
    """Fine-grained top-4 MoE baseline with more smaller routed experts."""
    ...
    def __init__(
        ...
        *,
        ...
        if num_shared_experts != 0:
            raise ValueError("FineGrainedMoENLIModel does not use shared experts.")
    ...
    def forward(self, features: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        routed_output = combine_topk_expert_outputs(
            flat,
            self.routed_experts,
            routing["topk_indices"],
            routing["topk_weights"],
        )
        moe_output = routed_output.reshape(batch_size, num_modes, self.hidden_dim)
